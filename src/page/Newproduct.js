    ...
    <div className="p-5">
       <form className='m-auto w-full max-w-md  shadow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <div  className='h-40 w-full bg-slate-200  rounded flex items-center justify-center cursor-pointer'>
           ...
           <input type={"file"} accept="image/*" id="image" onChange="" className="hidden"/>
        </div>
        </label>
        <label htmlFor='price' className='my-1'>Price</label>
        <input type={"text"} className='bg-slate-200 p-1 my-1' name='price' onChange={handleOnChange} value={data.price}/>
        <label htmlFor='description'>Description</label>
        <textarea rows={5} value={data.description} className='bg-slate-200 p-2 my-2 resize-none' name='description' onChange={handleOnChange}></textarea>
        <button className='bg-red-400 text-white text-lg font-medium my-1 drop-shadow'>Save</button>
       </form>
    </div>
  )
}
